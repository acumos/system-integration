...
setup_keystore() {
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
    if [[ ! -e /mnt/$ACUMOS_NAMESPACE/certs ]]; then
      setup_docker_volume /mnt/$ACUMOS_NAMESPACE/certs "$ACUMOS_HOST_USER:$ACUMOS_HOST_USER"
    fi
...
function setup_docker_engine_on_host() {
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" && "$ACUMOS_DOCKER_REGISTRY_HOST" == "$ACUMOS_INTERNAL_NEXUS_HOST" ]]; then
    update_nexus_env ACUMOS_DOCKER_REGISTRY_HOST $ACUMOS_HOST force
  fi
...
function prepare_env() {
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
    ...
    set_k8s_env
    if [[ "$K8S_DIST" == "openshift" ]]; then
      oc login -u $ACUMOS_OPENSHIFT_USER -p $ACUMOS_OPENSHIFT_PASSWORD
...
FAIL_REASON=
EOF
update_acumos_env DEPLOY_RESULT prepped force
