...
function setup_peer() {
  ...
  if [[ "$DEPLOYED_UNDER" == "k8s" ]]; then
    pod=$(kubectl get pods --namespace default | awk '/federation/ {print $1}')
    kubectl exec $pod -- /bin/sh -c "echo $ip $subjectName >>/etc/hosts"
  else
  ...
  if [[ "$DEPLOYED_UNDER" == "k8s" ]]; then
    pod=$(kubectl get pods --namespace default | awk '/federation/ {print $1}')
    kubectl delete -f kubernetes/federation-deployment.yaml
    kubectl create -f kubernetes/federation-deployment.yaml
  else
