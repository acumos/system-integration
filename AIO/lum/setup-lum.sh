...
function setup_lum_k8() {
  trap 'fail' ERR
    ...
    cd kubernetes/license-usage-manager
    git fetch "https://gerrit.acumos.org/r/license-usage-manager" refs/changes/69/5669/1 && git checkout FETCH_HEAD
    cd ../../
...
LUM_EXTERNAL_NAME=$LUM_RELEASE_NAME-$LUM_CHART_NAME-$LUM_RELEASE_NAME.svc.cluster.local
