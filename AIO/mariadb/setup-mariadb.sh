...
if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
  ...
  source $AIO_ROOT/setup-pv.sh setup pvc mariadb-data $ACUMOS_MARIADB_NAMESPACE \
    $MARIADB_DATA_PV_SIZE

  if [[ "$K8S_DIST" == "openshift" ]]; then
    log "Workaround variation in OpenShift for external access to mariadb"
    sed -i -- 's/<ACUMOS_ADMIN_HOST>/172.17.0.1/' kubernetes/mariadb-deployment.yaml
  fi
...
i=0
while ! mysql -h $ACUMOS_HOST_IP -P $ACUMOS_MARIADB_PORT --user=root \
