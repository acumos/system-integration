...
ACUMOS_PROJECT_NEXUS_PASSWORD ACUMOS_PROJECT_NEXUS_USERNAME ACUMOS_RW_USER \
ACUMOS_RW_USER_PASSWORD AZURE_CLIENT_IMAGE BLUEPRINT_ORCHESTRATOR_IMAGE \
...
sed -i -- '/K8S_DIST/d' acumos-env.sh

if [[ ! $(kubectl get namespaces) ]]; then
  DEPLOYED_UNDER=docker
else
  DEPLOYED_UNDER=k8s
  if [[ ! $(oc get namespaces) ]]; then
    K8S_DIST=generic
  else
    K8S_DIST=openshift
  fi
...
if [[ "$ACUMOS_CDS_PREVIOUS_VERSION" == "" ]]; then
  ...
  echo "export ACUMOS_RO_USER_PASSWORD=\"$ACUMOS_RO_USER_PASSWORD\"" >>acumos-env.sh
  ACUMOS_RW_USER_PASSWORD=$(uuidgen)
  echo "export ACUMOS_RW_USER_PASSWORD=\"$ACUMOS_RW_USER_PASSWORD\"" >>acumos-env.sh
