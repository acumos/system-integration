...
function setup_jenkins() {
  ...
  if [[ $(nc -w 5 -z $ACUMOS_NAMESPACE-jenkins 8080) -eq 0 ]]; then
  ...
  for f in $fs; do
    ...
    curl -v -X POST ${url}createItem?name=$job \
  ...
  curl -v -X POST ${url}job/security-verification-scan/build \
...
function customize_catalog() {
  ...
  if [[ $(nc -w 5 -z cds-service 8000) -eq 0 ]]; then
...
function setup_federation() {
  ...
  local creds="$ACUMOS_CDS_USER:$ACUMOS_CDS_PASSWORD"
  local t=0
  while [[ $(curl -u $creds $cds_baseurl/peer | grep -c numberOfElements) -eq 0 ]]; do
