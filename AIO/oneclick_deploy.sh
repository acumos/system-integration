...
function clean_env() {
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
    ...
    trap '' ERR
    if [[ "$k8s_cmd" == "kubectl" ]]; then what=namespace
    else what=project
    fi
    echo "Clean any existing install by deleting $what $ACUMOS_NAMESPACE"
    $k8s_cmd delete $what $ACUMOS_NAMESPACE
    while $k8s_cmd get $what $ACUMOS_NAMESPACE; do
      echo "Waiting 10 seconds for $what $ACUMOS_NAMESPACE to be deleted"
      sleep 10
    done
    trap 'fail' ERR
  fi
...
function prepare_env() {
  ...
  if [[ "$DEPLOYED_UNDER" == "k8s" ]]; then
    if [[ "$k8s_cmd" == "kubectl" ]]; then
      ...
      while ! $k8s_cmd create namespace $ACUMOS_NAMESPACE; do
...
EOF
    ...
    vars="ACUMOS_AZURE_CLIENT_PORT ACUMOS_CDS_DB ACUMOS_CDS_PASSWORD ACUMOS_CDS_PORT ACUMOS_CDS_USER ACUMOS_CMS_PORT ACUMOS_DATA_BROKER_INTERNAL_PORT ACUMOS_DATA_BROKER_PORT ACUMOS_DEPLOYED_SOLUTION_PORT ACUMOS_DEPLOYED_VM_PASSWORD ACUMOS_DEPLOYED_VM_USER ACUMOS_DOCKER_API_PORT ACUMOS_DOCKER_MODEL_PORT ACUMOS_DOCKER_PROXY_PORT ACUMOS_DOMAIN ACUMOS_DSCE_PORT ACUMOS_ELK_KIBANA_HOST ACUMOS_ELK_KIBANA_NODEPORT ACUMOS_FEDERATION_PORT ACUMOS_HOST ACUMOS_KEYPASS ACUMOS_KONG_PROXY_SSL_PORT ACUMOS_KUBERNETES_CLIENT_PORT ACUMOS_MARIADB_HOST ACUMOS_MARIADB_PORT ACUMOS_MARIADB_USER_PASSWORD ACUMOS_MICROSERVICE_GENERATION_PORT ACUMOS_NAMESPACE ACUMOS_NEXUS_API_PORT ACUMOS_NEXUS_HOST ACUMOS_ONBOARDING_PORT ACUMOS_OPERATOR_ID ACUMOS_PORTAL_BE_PORT ACUMOS_PORTAL_FE_PORT ACUMOS_PROBE_PORT ACUMOS_PROJECT_NEXUS_PASSWORD ACUMOS_PROJECT_NEXUS_USERNAME ACUMOS_RW_USER ACUMOS_RW_USER_PASSWORD AZURE_CLIENT_IMAGE BLUEPRINT_ORCHESTRATOR_IMAGE COMMON_DATASERVICE_IMAGE DATABROKER_CSVBROKER_IMAGE DATABROKER_SQLBROKER_IMAGE DATABROKER_ZIPBROKER_IMAGE DESIGNSTUDIO_IMAGE FEDERATION_IMAGE HTTP_PROXY HTTPS_PROXY KUBERNETES_CLIENT_IMAGE MICROSERVICE_GENERATION_IMAGE ONBOARDING_BASE_IMAGE ONBOARDING_IMAGE PORTAL_BE_IMAGE PORTAL_CMS_IMAGE PORTAL_FE_IMAGE PROTO_VIEWER_IMAGE"
...
if [[ "$ACUMOS_CDS_PREVIOUS_VERSION" == "" ]]; then
  ...
  AIO_ROOT=$WORK_DIR
  echo "AIO_ROOT=\"$AIO_ROOT\"" >>acumos-env.sh
  echo "export AIO_ROOT" >>acumos-env.sh
...
echo "/tmp/acumos, e.g. for debugging or if you are really bored."
...
Onboarding API: https://$ACUMOS_DOMAIN:$ACUMOS_KONG_PROXY_SSL_PORT/onboarding-app
