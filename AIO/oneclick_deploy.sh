...
function prepare_env() {
  ...
  bash setup-pv.sh setup pv $ACUMOS_LOGS_PV_NAME $ACUMOS_LOGS_PV_SIZE "$USER:$USER"
  bash setup-pv.sh setup pv $ACUMOS_OUTPUT_PV_NAME $ACUMOS_OUTPUT_PV_SIZE "$USER:$USER"
  bash setup-pv.sh setup pv $ACUMOS_WEBONBOARDING_PV_NAME $ACUMOS_WEBONBOARDING_PV_SIZE "$USER:$USER"
  bash setup-pv.sh setup pv $ACUMOS_CERTS_PV_NAME $ACUMOS_CERTS_PV_SIZE "$USER:$USER"
}
...
export WORK_DIR=$(pwd)
AIO_ROOT=$WORK_DIR
sed -i -- "/AIO_ROOT=.*/AIO_ROOT=$AIO_ROOT/" acumos-env.sh
...
sed -i -- "/DEPLOYED_UNDER=.*/DEPLOYED_UNDER=$DEPLOYED_UNDER/" acumos-env.sh
if [[ "$2" == "openshift" ]]; then
  K8S_DIST=openshift
  k8s_cmd=oc
else
...
sed -i -- "/K8S_DIST=.*/K8S_DIST=$K8S_DIST/" acumos-env.sh
