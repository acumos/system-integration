...
function prepare_env() {
  ...
  if [[ "$DEPLOYED_UNDER" == "k8s" ]]; then
    ...
    logs="$ACUMOS_COMMON_LOGS_PVC_NAME $ACUMOS_PORTAL_LOGS_PVC_NAME $ACUMOS_ONBOARDING_LOGS_PVC_NAME $ACUMOS_DEPLOYMENT_LOGS_PVC_NAME"
    for log in $logs; do
      if [[ "$log" != "$ACUMOS_LOGS_PVC_NAME" ]]; then
        setup_pvc $ACUMOS_NAMESPACE $log $log $ACUMOS_LOGS_PV_SIZE
      fi
    done
