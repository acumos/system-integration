...
{
  "insecure-registries": [
    "$ACUMOS_NEXUS_HOST:$ACUMOS_NEXUS_API_PORT", "$ACUMOS_NEXUS_HOST:$ACUMOS_NEXUS_DOCKER_REPO1_PORT", "$ACUMOS_NEXUS_HOST:$ACUMOS_NEXUS_DOCKER_REPO2_PORT", "$ACUMOS_NEXUS_HOST:$ACUMOS_NEXUS_DOCKER_REPO3_PORT"
  ],
...
EOF
  ...
  if [[ $check -eq 0 ]]; then
    sudo sed -i -- "s~ExecStart=/usr/bin/dockerd~ExecStart=/usr/bin/dockerd --insecure-registry localhost:4243~" /lib/systemd/system/docker.service
...
function setup_reverse_proxy() {
  ...
  keytool -genkeypair -keystore certs/acumos.jks -storepass password \
   ...
   -ext "SAN=DNS:$ACUMOS_HOSTNAME,DNS:$ACUMOS_DOCKER_MODEL_REPO,DNS:acumos"
  ...
  curl -i -X POST http://$ACUMOS_KONG_ADMIN_HOST:$ACUMOS_KONG_ADMIN_PORT/certificates \
    ...
    -F "snis=$ACUMOS_KONG_ADMIN_HOST,$ACUMOS_DOCKER_MODEL_REPO"
  ...
  curl -i -X POST \
    ...
    --data "name=root" \
    --data "upstream_url=http://$ACUMOS_HOSTNAME:$ACUMOS_PORTAL_FE_PORT" \
    --data "uris=/" \
    --data "strip_uri=false"
  curl -i -X POST \
    --url http://$ACUMOS_KONG_ADMIN_HOST:$ACUMOS_KONG_ADMIN_PORT/apis/ \
    --data "name=ccds" \
    --data "upstream_url=http://$ACUMOS_HOSTNAME:$ACUMOS_CDS_PORT/ccds" \
    --data "uris=/ccds" \
    --data "strip_uri=false"
  curl -i -X POST \
    --url http://$ACUMOS_KONG_ADMIN_HOST:$ACUMOS_KONG_ADMIN_PORT/apis/ \
  ...
  curl -i -X POST \
    --url http://$ACUMOS_KONG_ADMIN_HOST:$ACUMOS_KONG_ADMIN_PORT/apis/ \
    --data "name=nexus" \
    --data "upstream_url=http://$ACUMOS_NEXUS_HOST:$ACUMOS_NEXUS_API_PORT/repository" \
    --data "uris=/repository" \
    --data "strip_uri=false"
  curl -i -X POST \
    --url http://$ACUMOS_KONG_ADMIN_HOST:$ACUMOS_KONG_ADMIN_PORT/apis/ \
    --data "name=$ACUMOS_DOCKER_MODEL_REPO" \
    --data "upstream_url=http://$ACUMOS_NEXUS_HOST:$ACUMOS_DOCKER_MODEL_PORT" \
    --data "hosts=$ACUMOS_DOCKER_MODEL_REPO" \
    --data "strip_uri=false"
  ...
  sudo mkdir -p /etc/docker/certs.d/$ACUMOS_DOCKER_MODEL_REPO
  sudo cp certs/acumos.cert /etc/docker/certs.d/$ACUMOS_DOCKER_MODEL_REPO/ca.crt
}
...
$ACUMOS_HOST $ACUMOS_DOCKER_MODEL_REPO
$ACUMOS_HOST $ACUMOS_DOCKER_PLATFORM_REPO
$ACUMOS_HOST $ACUMOS_MAVEN_MODEL_REPO
$ACUMOS_HOST $ACUMOS_NEXUS_HOSTNAME
$ACUMOS_HOST $ACUMOS_CDS_HOSTNAME
$ACUMOS_HOST $ACUMOS_PORTAL_FE_HOSTNAME
$ACUMOS_HOST $ACUMOS_ONBOARDING_HOSTNAME
EOF
}
