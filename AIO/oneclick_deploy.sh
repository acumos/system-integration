...
function setup_keystore() {
  ...
  openssl req -x509 -new -nodes -key certs/acumos.key -sha256 -days 1024 \
    -config openssl.cnf -out certs/acumos.crt -passin pass:$ACUMOS_KEYPASS \
    -subj "/C=US/ST=Unspecified/L=Unspecified/O=Acumos/OU=Acumos/CN=$ACUMOS_HOST/subjectAltName=$ACUMOS_DOMAIN"
...
function setup_reverse_proxy() {
  ...
  sudo cp certs/acumos.crt /etc/docker/certs.d/$ACUMOS_HOST/ca.crt
}
