...
msg onboarding portal-be portal-fe"
    for dep in $deps; do
...
portal-be-service portal-fe-service"
    for svc in $svcs; do
...
function setup_acumos() {
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
    if [[ "$ACUMOS_DEPLOY_KONG" != "true" ]]; then
      ...
      update_env ACUMOS_PORTAL_FE_PORT $ACUMOS_PORTAL_FE_NODEPORT force
      export ACUMOS_ONBOARDING_PORT=$ACUMOS_ONBOARDING_NODEPORT
      update_env ACUMOS_ONBOARDING_PORT $ACUMOS_PORTAL_FE_NODEPORT force
      update_env ACUMOS_PORTAL_FE_PORT $ACUMOS_ONBOARDING_NODEPORT force
...
function setup_federation() {
  ...
  wait_until_success \
    "curl -s -o $HOME/json -u $ACUMOS_CDS_USER:$ACUMOS_CDS_PASSWORD http://$ACUMOS_CDS_HOST:$ACUMOS_CDS_PORT/ccds/peer"
...
if [[ -e mariadb-env.sh ]]; then
  ...
  if [[ ! $(grep '^source mariadb-env.sh' acumos-env.sh) ]]; then
...
if [[ "$ACUMOS_DEPLOY_KONG" == "true" ]]; then
  cd kong; source setup-kong.sh; cd $AIO_ROOT
fi
