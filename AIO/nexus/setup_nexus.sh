...
function deploy_nexus_service() {
  ...
  if [[ "$ACUMOS_INGRESS_LOADBALANCER" == "true" ]]; then
    if [[ "$ACUMOS_NEXUS_DOMAIN" == "$ACUMOS_DOMAIN" ]]; then
      # Each loadBalancer needs a distinct domain name
      update_nexus_env ACUMOS_NEXUS_DOMAIN nexus-$ACUMOS_DOMAIN force
    fi
  ...
  if [[ "$ACUMOS_INGRESS_LOADBALANCER" == "true" ]]; then
    ...
    ACUMOS_NEXUS_HOST_IP=$(kubectl get svc nexus-service -o json | jq -r '.status.loadBalancer.ingress[0].ip')
