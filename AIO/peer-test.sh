...
#
# Usage:
# $ bash peer-test.sh <host1> <ip1> <user1> <under1> <host2> <ip2> <user2> <under2> [models]
...
user1=$3
under1=$4
if [[ "$under1" != "docker" ]]; then
  if [[ "$under1" == "k8s" ]]; then k8s_dist1=generic
  else
    under1=k8s
    k8s_dist1=openshift
  fi
...
user2=$7
under2=$8
if [[ "$under2" != "docker" ]]; then
  if [[ "$under2" == "k8s" ]]; then k8s_dist2=generic
  else
    under2=k8s
    k8s_dist2=openshift
  fi
