...
if [[ "$K8S_DIST" == "openshift" ]]; then
  wget https://raw.githubusercontent.com/openshift/origin/master/examples/sample-app/cleanup.sh
  bash cleanup.sh
...
if [[ ! $(sudo rm -rf /var/lib/docker) ]]; then
  echo "Warning: all docker data cold not be deleted"
fi
