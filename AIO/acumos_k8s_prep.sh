...
for pv in $pvs; do
  if [[ ! $(kubectl get pv $pv) ]]; then
    bash setup-pv.sh setup $HOSTNAME $USER $pv /var/$ACUMOS_NAMESPACE 5Gi
  fi
done
...
source setup-mariadb.sh $(hostname) generic 2>&1 | tee ~/$ACUMOS_NAMESPACE/logs/$log
...
sedi "s/ACUMOS_NAMESPACE=.*/ACUMOS_NAMESPACE=$ACUMOS_NAMESPACE/" acumos-env.sh
