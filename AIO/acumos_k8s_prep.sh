...
export PREP_ROOT=$(pwd)

mkdir -p acumos/env acumos/logs acumos/certs
...
if [[ ! $(kubectl get nodes) ]]; then
  cd $PREP_ROOT/system-integration/tools
  bash setup_k8s.sh
  bash setup_helm.sh
...
for pv in $pvs; do
  if [[ ! $(kubectl get pv $pv) ]]; then
    bash setup-pv.sh setup $HOSTNAME $USER $pv /var/acumos 5Gi
  fi
done
...
# mariadb setup
cd $PREP_ROOT/system-integration/charts/mariadb
...
source setup-mariadb.sh $(hostname) generic 2>&1 | tee $PREP_ROOT/acumos/logs/$log
cp mariadb-env.sh $PREP_ROOT/acumos/env/.

# elk setup
source $PREP_ROOT/acumos/env/mariadb-env.sh
cd $PREP_ROOT/system-integration/charts/elk-stack
cat <<EOF >elk-env.sh
...
stamp=$(date +"%y%m%d-%H%M%S")
log="elk-$stamp.log"
source setup-elk.sh generic 2>&1 | tee $PREP_ROOT/acumos/logs/$log
cp elk-env.sh $PREP_ROOT/acumos/env/.

# Install the prerequisites
cd $PREP_ROOT/system-integration/AIO
sedi "s/ACUMOS_DEPLOY_MARIADB=true/ACUMOS_DEPLOY_MARIADB=false/" acumos-env.sh
sedi "s/ACUMOS_DEPLOY_ELK=true/ACUMOS_DEPLOY_ELK=false/" acumos-env.sh
sedi "s/ACUMOS_CREATE_CERTS=.*/ACUMOS_CREATE_CERTS=false/" acumos-env.sh
cp $PREP_ROOT/acumos/env/mariadb-env.sh .
cp $PREP_ROOT/acumos/env/elk-env.sh .
cp -r $PREP_ROOT/acumos/certs .
stamp=$(date +%y%m%d-%H%M%S)
log="aio_k8s_prep-$stamp.log"
bash setup_prereqs.sh k8s $ACUMOS_DOMAIN \
  $ACUMOS_HOST_USER generic 2>&1 | tee $PREP_ROOT/acumos/logs/$log
sudo chown -R $ACUMOS_HOST_USER:$ACUMOS_HOST_USER /var/$ACUMOS_NAMESPACE/*
cp acumos-env.sh $PREP_ROOT/acumos/env/.

if [[ "$ACUMOS_HOST_USER" != "$USER" ]]; then
  # Setup the acumos user env
  sudo cp -R $PREP_ROOT/.kube /home/$ACUMOS_HOST_USER/.
  sudo chown -R $ACUMOS_HOST_USER:$ACUMOS_HOST_USER /home/$ACUMOS_HOST_USER/.kube
  sudo cp $PREP_ROOT/$ACUMOS_HOST_USER/env/*-env.sh /home/$ACUMOS_HOST_USER/acumos/env/.
