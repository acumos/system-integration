...
clean_nifi() {
  ...
  if [[ $(kubectl delete configmap -n $ACUMOS_NAMESPACE nifi-certs) ]]; then
    log "configmap nifi-certs deleted"
  fi
...
setup_nifi() {
  ...
  mv $AIO_ROOT/certs/nifi* .
  mv $AIO_ROOT/certs/cert_env.sh  .
  source cert_env.sh
  ...
  cd ..
  kubectl create configmap -n $ACUMOS_NAMESPACE nifi-certs \
    --from-file=$AIO_ROOT/certs/$ACUMOS_KEYSTORE,$AIO_ROOT/certs/$ACUMOS_TRUSTSTORE
