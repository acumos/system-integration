...
setup_nifi() {
  ...
  MLWB_NIFI_KEYSTORE_PASSWORD=$(uuidgen)
  ...
  MLWB_NIFI_TRUSTSTORE_PASSWORD=$(uuidgen)
  ...
  $k8s_cmd create secret generic nifi-certs-registry -n $ACUMOS_NAMESPACE -o yaml \
    --from-file=apache_key.pem,apache_cert.pem,apache_proxy.pem,nifi-truststore.jks,nifi-keystore.p12
...
#  sedi "s/<CERT_KEY_PASSWORD>/$CERT_KEY_PASSWORD/g" deploy/templates/nifi_configmap.yaml
  sedi "s/<CERT_KEY_PASSWORD>//g" deploy/templates/nifi_configmap.yaml
  ...
  bash $AIO_ROOT/../tests/create_user.sh $AIO_ROOT/acumos_env.sh nifiadmin \
    $MLWB_NIFI_REGISTRY_INITIAL_PASSWORD \
