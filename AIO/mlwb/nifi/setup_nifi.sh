...
clean_nifi() {
  ...
  deps=$($k8s_cmd get deployment -n $ACUMOS_NAMESPACE  | awk '/nifi/{print $1}')
  for dep in $deps; do
  ...
  secrets=$($k8s_cmd get secret -n $ACUMOS_NAMESPACE | awk '/nifi/{print $1}')
  for secret in $secrets; do
    $k8s_cmd delete secret -n $ACUMOS_NAMESPACE $secret
  done
