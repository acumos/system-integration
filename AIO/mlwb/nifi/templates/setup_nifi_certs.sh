...
setup_nifi_certs() {
  ...
  log "Import $MLWB_NIFI_REGISTRY_INITIAL_ADMIN_NAME cert into truststore with CA cert"
  keytool -import -noprompt \
    -file /maven/conf/apache_cert.pem \
  ...
  kubectl create secret generic nifi-certs-$nifiuser -n $ACUMOS_NAMESPACE -o yaml \
    --from-file=apache_key.pem,apache_cert.pem,apache_proxy.pem,nifi-truststore.jks,nifi-keystore.p12,nifi-registry-keystore.p12
}
