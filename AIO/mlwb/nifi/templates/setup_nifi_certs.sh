...
setup_nifi_certs() {
  ...
  keytool -trustcacerts -import -noprompt \
  ...
  kubectl create secret generic nifi-certs-$nifiuser -n $ACUMOS_NAMESPACE -o yaml \
    --from-file=apache_key.pem,apache_cert.pem,apache_proxy.pem,nifi_cert.pem,nifi-truststore.jks,nifi-keystore.p12,nifi-registry-keystore.p12
}
