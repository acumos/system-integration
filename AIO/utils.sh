...
function get_pv_claim_refs() {
  ...
  if [[ "$(kubectl get pv $name -o json | jq -r ".spec.claimRef.name")" != "null" ]]; then
    ...
    pv_claim_refs=$(jq -c '.items[] | {name: .metadata.name, namespace: .metadata.namespace, claimName: .spec | select( has ("volumes") ).volumes[] | select( has ("persistentVolumeClaim") ).persistentVolumeClaim.claimName }' $tmp | grep $pv_claim)
    rm $tmp
  fi
}
