...
function wait_until_null() {
  cmd="$1"
  log "Waiting for \"$cmd\" to return no result"
  while [[ "$($cmd)" != "" ]]; do
    log "Command \"$cmd\" returned a result, waiting 10 seconds"
    sleep 10
  done
}
...
function stop_service() {
  if [[ -e $1 ]]; then
    ...
    if [[ $($k8s_cmd get svc -n $ACUMOS_NAMESPACE -l app=$app) ]]; then
      ...
      $k8s_cmd delete -f $1
      wait_until_null "$k8s_cmd get svc -n $ACUMOS_NAMESPACE | grep $app"
    else
...
function stop_deployment() {
  if [[ -e $1 ]]; then
    ...
    if [[ $($k8s_cmd get deployment -n $ACUMOS_NAMESPACE -l app=$app) ]]; then
      ...
      $k8s_cmd delete -f $1
      wait_until_null "$k8s_cmd get pods -n $ACUMOS_NAMESPACE | grep $app"
    else
