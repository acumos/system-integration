...
function start_acumos_core_app() {
  ...
  if [[ "$HOST_IP" != "" ]]; then
    patch_template_with_host_alias deploy/$app-service.yaml $ACUMOS_HOST $HOST_IP
  fi
  if [[ "$app" == "cds" && "$ACUMOS_MARIADB_HOST" != "$ACUMOS_HOST" ]]; then
    get_host_ip_from_etc_hosts $ACUMOS_MARIADB_HOST
    if [[ "$HOST_IP" != "" ]]; then
      patch_template_with_host_alias deploy/$app-service.yaml $ACUMOS_MARIADB_HOST $HOST_IP
    fi
  fi
