...
function setup() {
  ...
  log "Setup Acumos databases via mariadb server"
  if [[ "$ACUMOS_CDS_PREVIOUS_VERSION" == "" ]]; then
    ...
    mysql $server --user=root --password=$ACUMOS_MARIADB_PASSWORD -e "USE $ACUMOS_CDS_DB; GRANT ALL PRIVILEGES ON $ACUMOS_CDS_DB.* TO 'acumos_opr'@'%' IDENTIFIED BY \"$ACUMOS_MARIADB_USER_PASSWORD\";"
    ...
    mysql $server --user=acumos_opr --password=$ACUMOS_MARIADB_USER_PASSWORD < cmn-data-svc-ddl-dml-mysql-$ACUMOS_CDS_VERSION.sql

    log "Setup database 'acumos_cms'"
    mysql $server --user=root --password=$ACUMOS_MARIADB_PASSWORD -e "CREATE DATABASE acumos_cms; USE acumos_cms; GRANT ALL PRIVILEGES ON acumos_cms.* TO 'acumos_opr'@'%' IDENTIFIED BY \"$ACUMOS_MARIADB_USER_PASSWORD\";"
  else
    if [[ "$ACUMOS_CDS_PREVIOUS_VERSION" != "$ACUMOS_CDS_VERSION" ]]; then
      ...
      sed -i -- "1s/^/use $ACUMOS_CDS_DB;\n/" $upgrade
      mysql $server --user=acumos_opr --password=$ACUMOS_MARIADB_USER_PASSWORD < $upgrade
    fi
  fi
}
