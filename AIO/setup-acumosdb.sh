...
function setup() {
  ...
  if [[ "$ACUMOS_CDS_PREVIOUS_VERSION" == "" ]]; then
    ...
    mysql $server --user=root --password=$ACUMOS_MARIADB_PASSWORD -e "CREATE DATABASE acumos_cms; USE acumos_cms; GRANT ALL PRIVILEGES ON acumos_cms.* TO \"$ACUMOS_MARIADB_USER\"@'%' IDENTIFIED BY \"$ACUMOS_MARIADB_USER_PASSWORD\";"
