...
function setup_jenkins() {
  trap 'fail' ERR
  bash $AIO_ROOT/../charts/jenkins/setup_jenkins.sh all $ACUMOS_NAMESPACE $ACUMOS_ORIGIN
  local pod=$(kubectl get pods -n $ACUMOS_NAMESPACE | awk '/jenkins/{print $1}')

  log "Copy default kube-config into Jenkins pod at /var/jenkins_home/.kube/config"
  if oc exec -it -n $ACUMOS_NAMESPACE $pod -- rm -rf /var/jenkins_home/.kube; then
    log "Prior /var/jenkins_home/.kube/config was removed"
  fi
  ...
  if [[ ! -e deploy/jobs/initial-setup.xml ]]; then
    cp $AIO_ROOT/../charts/jenkins/jobs/initial-setup.xml deploy/jobs/.
  fi
