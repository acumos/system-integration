...
function setup_jenkins() {
  ...
  log "Copy default kube-config into /var/jenkins_home/.kube/config"
  kubectl exec -it -n $ACUMOS_NAMESPACE $pod -- mkdir -p /var/jenkins_home/.kube
  ...
  cp deploy/security-verification/jenkins/security-verification-scan.xml \
    deploy/jenkins/jobs/security-verification-scan.xml
  ...
  t=0
  while [[ "$(kubectl exec -it $pod -- ls /var/jenkins_home/workspace/security-verification-scan)" != *"sv"* ]]; do
  ...
  mkdir -p deploy/jenkins/security-verification-scan
  grep ACUMOS_CDS $AIO_ROOT/acumos_env.sh >deploy/jenkins/security-verification-scan/acumos_env.sh
  grep ACUMOS_SECURITY_VERIFICATION_PORT $AIO_ROOT/acumos_env.sh >>deploy/jenkins/security-verification-scan/acumos_env.sh
  grep ACUMOS_NEXUS $AIO_ROOT/nexus_env.sh >>deploy/jenkins/security-verification-scan/acumos_env.sh
  cp -r deploy/security-verification/jenkins/scan/* deploy/jenkins/security-verification-scan
  kubectl cp deploy/jenkins/security-verification-scan -n $ACUMOS_NAMESPACE $pod:/var/jenkins_home/workspace/security-verification-scan

  log "Execute security-verification-scan setup job to initialize scancode utility"
  curl -v -X POST ${url}job/security-verification-scan/build $auth \
    --data-urlencode json='{"parameter":[{"name":"solutionId","value":""},{"name":"revisionId","value":""},{"name":"userId","value":""}]}'
}
