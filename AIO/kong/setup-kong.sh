...
function setup() {
  ...
  log "Stop kong-service components"
...
#    sudo bash docker-compose.sh $AIO_ROOT down
    ...
    log "Delete the kong-db PVC"
    bash $AIO_ROOT/setup-pv.sh clean pvc kong-db
  fi

  log "Setup the kong-db PV"
  bash $AIO_ROOT/setup-pv.sh setup pv kong-db \
    $PV_SIZE_KONG_DB "$USER:$USER"
  ...
  if [[ "$DEPLOYED_UNDER" == "docker" ]]; then
    ...
    replace_env deploy "ACUMOS_NAMESPACE ACUMOS_HOST ACUMOS_DOMAIN ACUMOS_KONG_DB_PORT \
      ACUMOS_KONG_PROXY_PORT ACUMOS_KONG_PROXY_SSL_PORT ACUMOS_KONG_ADMIN_PORT \
      ACUMOS_KONG_ADMIN_SSL_PORT"
