...
function elk_fail() {
  ...
  sedi 's/DEPLOY_RESULT=.*/DEPLOY_RESULT=fail/' elk-env.sh
  sedi "s/FAIL_REASON=.*~FAIL_REASON=$reason~/" elk-env.sh
  log "$reason"
  exit 1
}
...
sedi 's/DEPLOY_RESULT=.*/DEPLOY_RESULT=success/' elk-env.sh
