...
function setup_jenkins() {
  ...
  if [[ "$K8S_DIST" == "openshift" ]]; then
    ...
    update_acumos_env ACUMOS_JENKINS_RUNASUSER $OPENSHIFT_UID force
  fi
  ...
  if [[ "$K8S_DIST" == "generic" ]]; then
    ...
    # OpenShift default PV permissions are incompatible with Jenkins' init
    # container "copy-default-config".
    # TODO: fix for this workaround, e.g. a custom recycler that sets permissions
    # Customized chart is needed to enabler securityContext privileged, to enable
    # copy-default-config to setup the Jenkins config in the PV
