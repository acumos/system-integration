...
4. Clone the gerrit.acumos.org _system-integration_ repository into the new directory
5. Change directory into the newly created _system-integration_ directory
...
The *proxy.txt* file is located in the _z2a/distro-setup_ directory.  This file needs to be edited such that the Docker installation can proceed cleanly.  We will need to change directories into that location to perform the necessary edits required for the Acumos installation.

This file will contain a single entry in the form of _hostname_ OR _hostname:port_ (this is not a URL).
...
The *global_value.yaml* file is located in the _helm-charts_ directory noted above.  We will need to change directories into that location to perform the necessary edits required for the Acumos installation.
...
For entries in the _global_value.conf_ file that have a entry, do not edit these values as they are essential for correct installation.
...
1. change directory into the _z2a_ directory
2. execute the z2a script which install the MLWB dependencies and the MLWB components
