...
#   env: path to local platform environment file acumos-env.sh
...
function add_host_alias() {
  ...
  if [[ $(host $name | grep -c 'not found') -eq 0 ]]; then
    HOST_IP=$(host $name | head -1 | cut -d ' ' -f 4)
  elif [[ $(grep -c -P " $name( |$)" /etc/hosts) -gt 0 ]]; then
    HOST_IP=$(grep -P "$name( |$)" /etc/hosts | cut -d ' ' -f 1)
  else
  ...
  template:
    spec:
      ...
      - ip: "$ip"
        ...
        - "$name"
EOF
    kubectl patch deployment -n $ACUMOS_NAMESPACE -l app=$app --patch @$tmp
    rm $tmp
  else
    c=$(docker ps -a | awk "/$app/{print $1}")
    docker exec -it $c /bin/sh -c "echo \"$ip $host\" >>/etc/hosts"
  fi
fi
}
