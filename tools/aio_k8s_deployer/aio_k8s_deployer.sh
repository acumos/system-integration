...
function setup_prereqs() {
  ...
  if [[ "$status" != "prepped" ]]; then
    fail "Failure in setup_prereqs.sh: $reason"
  fi
}
...
function run_all() {
  trap 'fail' ERR
  if [[ -e ~/acumos/$ACUMOS_HOST/deploy ]]; then
    stamp=$(stat -c %y ~/acumos/$ACUMOS_HOST/deploy | cut -d '.' -f 1 | sed 's/ /-/' | sed 's/:/-/g')
    mv ~/acumos/$ACUMOS_HOST/deploy ~/acumos/$ACUMOS_HOST/deploy-$stamp
  fi
  mkdir -p ~/acumos/$ACUMOS_HOST/deploy
  cp -r * ~/acumos/$ACUMOS_HOST/deploy
  cd ~/acumos/$ACUMOS_HOST/deploy
  ...
  if [[ -e aio_k8s_deployer.sh ]]; then
    cp aio_k8s_deployer.sh ~/acumos/$ACUMOS_HOST/.
  else
    cp system-integration/tools/aio_k8s_deployer/aio_k8s_deployer.sh ~/acumos/$ACUMOS_HOST/.
  fi
  ...
  host_ip=$(awk "/$ACUMOS_HOST/{print \$1}" /etc/hosts)
  if [[ "$host_ip" != "" ]]; then
    add_host="add-host=$ACUMOS_HOST:$host_ip"
  fi
  bash aio_k8s_deployer.sh deploy $ACUMOS_HOST tag=$client $add_host
}
